...
def count_genders(people_dict):
    ...
    print("Genders")
    print(f"Number of males: {total}")
    print(f"Number of females: {count}")
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, values in marriages_dict.items():
        ...
        birth = husband[BIRTH_YEAR_INDEX]

       
        wife_key = values[WIFE_KEY_INDEX]
        wife = people_dict[wife_key]
        wife_name = wife[NAME_INDEX]
        start = wife[BIRTH_YEAR_INDEX]

        marriage = values[WEDDING_YEAR_INDEX]
        age = marriage - birth
        age_one = marriage - start

        year = values[WEDDING_YEAR_INDEX]
        
        print(f"{husband_name} {age} > {year} < {wife_name} {age_one}")
